...
public:
    ...
    std::string savePCDDirectory_single;
    string path;
    string path_kitti;
    string saveNodeBinDirectory;
...
public:
    mapOptimization()
    {
        ...
        path = dirpath + "pose.txt";
        path_kitti = dirpath + "pose_kitti.txt";
        ...
        int unused = system((std::string("exec rm -r ") + dirpath).c_str());
        unused = system((std::string("mkdir ") + dirpath).c_str());

        saveSCDDirectory = dirpath + "SCDs/"; // SCD: scan context descriptor 
        // unused = system((std::string("exec rm -r ") + saveSCDDirectory).c_str());
        unused = system((std::string("mkdir -p ") + saveSCDDirectory).c_str());

        saveNodePCDDirectory = dirpath + "Scans/";
        unused = system((std::string("mkdir -p ") + saveNodePCDDirectory).c_str());

        saveNodeBinDirectory = dirpath + "bin/";
        ...
        file_kitti.open(path_kitti, ios::out);
    }
    ...
    void visualizeGlobalMapThread()
    {
        ...
        const std::string kitti_format_pg_filename {dirpath + "optimized_poses.txt"};
        ...
        // pcl::io::savePCDFileASCII(dirpath + "trajectory.pcd", *cloudKeyPoses3D);
        // pcl::io::savePCDFileASCII(dirpath + "transformations.pcd", *cloudKeyPoses6D);
        ...
        pcl::io::savePCDFileASCII(dirpath + "sharp_cloud_map.pcd", *globalCornerCloudDS);
        // down-sample and save surf cloud
        downSizeFilterSurf.setInputCloud(globalSurfCloud);
        downSizeFilterSurf.filter(*globalSurfCloudDS);
        pcl::io::savePCDFileASCII(dirpath + "flat_cloud_map.pcd", *globalSurfCloudDS);
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        // pcl::io::savePCDFileASCII(savePCDDirectory_single + curr_scd_node_idx + ".pcd", *thisKeyFrameCloud);
        ...
        file << fixed   << transformTobeMapped[3] << " " << transformTobeMapped[4] << " " << transformTobeMapped[5] << " "
        ...
        Eigen::Matrix4d T;
        T(0,3) = transformTobeMapped[3];
        T(1,3) = transformTobeMapped[4];
        T(2,3) = transformTobeMapped[5];
        T.block<3,3>(0,0) = R;
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 4; ++j) {
                file_kitti << T(i, j);
